...
3. **Tests:** Colocated `tests/` subdirectory next to tested module (e.g., `domain/comparison/tests/test_trueskill.py`). By default run only the colocated suites for the module under change; the general suite at the root `tests/` directory (`test_general.py` — full command↔endpoint contract) runs only on explicit user prompt. Tests and stubs touch temp directories only (`tmp_path`) — never real `output/`, ranked roots, or `config/`.
