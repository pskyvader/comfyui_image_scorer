...
from ..compressed_image import compressed_image_response, image_cache
...
def test_png_served_as_smaller_webp(tmp_path: Path) -> None:
    im = Image.new("RGB", (256, 256), (200, 100, 50))
    src = _write_image(tmp_path, im, "img.png", "PNG")

    response = compressed_image_response(src, "img.png")
...
def test_animated_gif_served_as_webp(tmp_path: Path) -> None:
    ...
    response = compressed_image_response(src, "anim.gif")

    assert response.mimetype == "image/webp"
    assert len(response.data) > 0


def test_cache_served_second_call(tmp_path: Path) -> None:
    im = Image.new("RGB", (128, 128), (10, 20, 30))
    src = _write_image(tmp_path, im, "img.png", "PNG")

    first = compressed_image_response(src, "img.png")
    second = compressed_image_response(src, "img.png")

    assert second.data == first.data
    assert image_cache.get(("img.png", src.stat().st_mtime_ns, src.stat().st_size)) is not None
